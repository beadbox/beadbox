...
**Every landing also gets a security review**, separate from code review, and it is pinned to the exact commit that was reviewed: if you push to the branch after the review, the review starts over, so please rebase before you open the PR rather than after. The review can hold a landing until a specific change is made; when that happens we usually make the change ourselves and land it with your attribution, as above. The changes that reliably hold a landing: anything that adds a network listener or widens the app's IPC scope, changes how the bd CLI is invoked or how workspace files and paths are read, touches CI workflows, secrets, git hooks or dependency manifests, reaches the installer, updater or Homebrew cask, or edits files that AI agents read as instructions (CLAUDE.md, AGENTS.md, SKILL.md, llms.txt). Test code gets the same reading as production code, because it runs on our machines before it runs anywhere else.

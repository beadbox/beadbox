...
import { useTypeCatalog } from "@/hooks/use-type-catalog"
...
export function useWorkspaceLifecycle(opts: UseWorkspaceLifecycleOpts) {
  ...
  const {
    ...
    retryAvailableTypes,
    cancelTypeCatalog,
  } = useTypeCatalog(currentWorkspace)
  ...
  useEffect(() => {
    if (currentWorkspace) {
      ...
      // A switch before these resolve must not apply the old workspace's statuses.
      return () => {
        active = false
        cancelTypeCatalog()
      }

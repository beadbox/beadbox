...
test("moving a bead under another parent still rebuilds the tree", () => {
  const task = { id: "task-a", type: "task", title: "Task", parentId: "epic-a", children: [] } as unknown as Bead
  const initial = [
    { id: "epic-a", type: "epic", title: "Epic A", children: [task] },
    { id: "epic-b", type: "epic", title: "Epic B", children: [] },
  ] as Epic[]
  const loadEpics = mock(async () => {})
  ...
  // Same type, new parent: a hierarchy change the local patch cannot express.
  act(() => result.current.handleBeadUpdate({ ...task, parentId: "epic-b" }))
  expect(loadEpics).toHaveBeenCalledTimes(1)
})

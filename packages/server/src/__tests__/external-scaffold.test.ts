...
describe("external server scaffold", () => {
  ...
  test("a scaffold already on this endpoint is not rewritten, whatever bd's formatting", async () => {
    const { beadsDir, server } = await fixture()
    // As bd writes it: its own key order, two-space indent, no trailing newline.
    const bdMetadata = JSON.stringify(
      {
        database: "dolt",
        backend: "dolt",
        dolt_mode: "server",
    ...
    await writeFile(join(beadsDir, "metadata.json"), bdMetadata)
    await writeFile(join(beadsDir, "config.yaml"), "issue-prefix: gtp\n\ndolt.auto-start: false\n")
    await ensureExternalScaffold(beadsDir, server)
    expect(await readFile(join(beadsDir, "metadata.json"), "utf8")).toBe(bdMetadata)
    expect((await readdir(beadsDir)).filter((name) => name.includes("beadbox-before-external"))).toEqual([])
  })

  test("refuses to rewrite when the backup path is a pre-placed symlink", async () => {
    const { beadsDir, server } = await fixture()
    const outside = join(beadsDir, "..", "outside.json")
    await writeFile(outside, "untouched")
    await symlink(outside, join(beadsDir, "metadata.json.beadbox-before-external"))
    const before = await readFile(join(beadsDir, "metadata.json"), "utf8")
    await expect(ensureExternalScaffold(beadsDir, server)).rejects.toThrow("not a regular file")
    expect(await readFile(join(beadsDir, "metadata.json"), "utf8")).toBe(before)
    expect(await readFile(outside, "utf8")).toBe("untouched")
    expect((await lstat(join(beadsDir, "metadata.json.beadbox-before-external"))).isSymbolicLink()).toBe(true)
  })
})

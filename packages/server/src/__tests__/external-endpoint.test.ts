...
import { _resetWriter, _setWriter, restartWorkspaceSubscriptions, state } from "../handlers/subscribe-internals"
...
describe("external scaffold endpoint", () => {
  ...
  test("a subscription stopped while its poller is being recreated is not resurrected", async () => {
    root = await mkdtemp(join(tmpdir(), "beadbox-subscription-race-"))
    const path = join(root, ".beads")
    await mkdir(path)
    const subscriptionPath = join(path, "beads.db")
    const id = "external-poller-race"
    state.paths.set(id, subscriptionPath)
    state.detectors.set(id, { stop: async () => {} })
    // The new detector emits its synthetic initial event before it is returned,
    // so stopping from the writer lands exactly between create and install.
    let stopRequested = false
    _setWriter((line) => {
      if (!stopRequested && line.includes(id) && line.includes('"initial"')) {
        stopRequested = true
        void stopSubscription(id)
      }
    })
    try {
      await restartWorkspaceSubscriptions(path)
      expect(stopRequested).toBe(true)
      expect(state.paths.has(id)).toBe(false)
      expect(state.detectors.has(id)).toBe(false)
    } finally {
      _resetWriter()
      await state.detectors.get(id)?.stop()

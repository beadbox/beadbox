...
- **Archive hint toast stuck on screen**: the "move to archive" hint toast used to stay visible indefinitely. It now auto-dismisses after 10 seconds.
- **Archived section hidden in no-epic workspaces**: when a workspace had no epics, the Archived section would not render even if archived beads existed. It now appears whenever there is archived work to show.
- **Embedded-mode crash detection missed**: the health-polling effect skipped embedded mode, so a crashed Dolt process in embedded workspaces would not be surfaced until the next UI action. The effect is now mode-agnostic and catches embedded crashes.

### Changed

- **Windows build alignment**: aligned `tauri-plugin-dialog` to 2.7.0 across npm and Cargo to fix a Windows CI version-mismatch failure.

## [0.21.2] - 2026-04-06

### Fixed

- **Workspace detection for `embeddeddolt/` layouts (bd 0.63.3+)**: bd 0.63.3 introduced an `embeddeddolt/` directory layout that Beadbox did not recognize, so these workspaces failed to open. Workspace detection now understands both layouts, with a redirect fallback for older code paths.

## [0.21.1] - 2026-04-03

### Fixed

- **Server-backed workspace health check missing password**: the health check path did not inject the stored `BEADS_DOLT_PASSWORD`, so password-protected Dolt servers showed as "unreachable" even when credentials were saved. The password is now injected on every server-backed health probe.
- **Tauri v2 environment detection**: `isTauri()` was still checking for a Tauri v1 internal marker, which never resolved true in Tauri v2, disabling native-only code paths. Updated to check `__TAURI_INTERNALS__`.
- **Server-only workspace scaffold**: creating a local `.beads/` scaffold for server-only workspaces now happens as part of the health check, with password lookup resolved via `metadata.json` so first-connect succeeds.
- **SQL queries on server-only workspaces**: `serverSqlQuery` now reads the user from the workspace registry instead of hardcoding `root`, and server-only SQL queries go through a direct MySQL client instead of the bd subprocess path.
...
- **Remote workspace credential persistence**: server workspace credentials (host, port, database, password) are now stored in the OS keychain instead of plaintext config files, with an access-denied re-auth dialog when stored credentials stop working.
...
- **Windows improvements**: increased startup timeout to 90 seconds for slower machines, added dual-stack (IPv4/IPv6) support for WebSocket connections, switched to platform-native `open` for external links, and fixed Windows CI signing (sign.bat wrapper, signtool path exports, PowerShell build step to avoid MSYS2 spawn failures).

### Fixed

- **Custom status filter hiding beads**: beads with custom statuses were excluded from filtered views. Custom statuses now pass through all filter paths and appear in the filter dropdown.
...
- Runtime directory infrastructure: Beadbox no longer creates or manages runtime scratch directories outside the workspace. Workspace state is tracked through bd's own config and port files.
- macOS Intel (x64) downloads: the Intel Mac download option is no longer surfaced on the downloads page. Homebrew-tap installs on Intel hardware remain supported via the cask's `on_intel` branch.
...
Released and rolled back while stabilizing beads v1.0.0 support. All fixes from this line are included in v0.21.0.
...
- **Faster workspace scanner**: removed the legacy 1000-port DerivePort range scan from the add-workspace dialog. Server discovery now reads port files directly, completing in under 2 seconds instead of ~10 seconds.
...
- **Documentation for ephemeral ports**: quick-start and troubleshooting pages across all 11 languages now reference the `.beads/dolt-server.port` file instead of the retired 13307-14306 port range.

### Fixed

- **Comment deletion broken on bd 0.61**: bd 0.61 migrated comment IDs from auto-increment integers to UUIDs. `deleteComment()` rejected UUID strings with "Invalid comment ID", making comment deletion non-functional. Fixed to accept both UUID and legacy integer IDs with SQL injection prevention.
- **Comment change detection fragile with UUID PKs**: server-mode polling used `MAX(id)` on the comments table, which returns a meaningless lexicographic maximum for UUID primary keys. Replaced with `COUNT(*)`, which correctly detects comment additions and deletions.

### Removed

- **Dead idle-monitor code from E2E tests**: bd 0.59 removed the daemon infrastructure. Cleaned up references to the defunct idle-monitor process across the E2E test suite (pkill commands, sleep guards, race condition comments).
...
- **Formula E2E test suite**: Playwright end-to-end tests covering the Formulas tab lifecycle.
...
- **Decomposed bead-detail-panel**: split into sub-components and hooks with unit tests for each.
- **Decomposed activity-feed**: extracted pure logic into utility modules with unit tests.
- **Decomposed Tauri lib.rs**: extracted into focused Rust modules with unit tests.

### Fixed

- **Circuit breaker recovery**: dead Dolt processes are now auto-recovered on startup, and WebSocket polling uses exponential backoff instead of a fixed 2-second interval that reset the circuit breaker cooldown.
...
- **Scanner accuracy**: dropped legacy default ports, tightened DerivePort scan range, and added Dolt verification to prevent false positives.
...
- **WebView height**: switched resizable panels from `h-full` to flex layout to prevent WebView height collapse.
...
- **Linux browser tab on launch**: prevented a stray browser tab from opening on Linux launch.
...
- **E2E test stability**: fixed port-scan dialog tests, workspace removal race conditions, delete test timing, and serialized test jobs to prevent Dolt cross-kill.
...
- **Server workspace connection via environment variables**: server-only workspaces now connect using bd's native env vars (`BEADS_DOLT_SERVER_HOST`, `BEADS_DOLT_SERVER_PORT`, `BEADS_DOLT_DATABASE`) instead of synthesizing runtime directories. Simpler config, fewer moving parts.
...
- **mysql2 connection pool**: Beadbox now maintains a persistent mysql2 connection pool per workspace instead of spawning a new `bd` process for every query. This is the foundation for eliminating subprocess overhead across the app. The ws-manager fingerprint poll is the first consumer: real-time update detection dropped from ~1400ms (Go process spawn + TCP connect + query + exit) to ~5ms (single query on a persistent connection).
